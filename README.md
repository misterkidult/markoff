...
**[開 demo 玩玩看 →](https://markoff.coreplay.tw)**
...
這支工具是從真實接案流程長出來的，幾個看起來奇怪的決定都有來由：

**沒有送出鈕。** 累積數則再一次送出，最容易漏掉那個動作。按下留言就是送出。

**刪除建議標記作廢。** 真刪會讓後面的資料位移，多人同時操作時會改到別人的留言。

**記錄「視窗佔螢幕多少」。** 只看 viewport 分不出「27 吋螢幕把視窗縮到 1200px」
和「筆電本來就 1200px」——這兩種情境的處理方式不同。

**iPhone 型號留空不猜。** 抓不到就是抓不到，寧可空著也不給一個看起來很確定的錯資訊。

**pin 位置以「元素現在在哪」為準**，不是留言當下的座標。頁面捲動、動畫、RWD 換行
都會讓元素移動，釘死座標會對不上。
